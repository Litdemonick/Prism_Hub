...
class TrackingPageController extends GetxController {
  ...
  updateAniListToken(String accessToken) {
    PrismHubStorage.setSetting(SettingKey.aniListToken, accessToken);
    anilistIsLogin.value = true;
    initAnilistData();
  }

  logoutAniList() {
    PrismHubStorage.setSetting(SettingKey.aniListToken, "");
  ...
  @override
  void onInit() {
    final token = PrismHubStorage.getSetting(SettingKey.aniListToken);
    if (token != "") {
      anilistIsLogin.value = true;
    }
    super.onInit();
