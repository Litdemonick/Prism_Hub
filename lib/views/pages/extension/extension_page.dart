...
import 'package:prismhub/views/pages/extension/extension_page_tv.dart';
...
///
/// Se bifurca acá y no en cada sitio que la abre — son cinco: la barra
/// lateral de escritorio, las dos de Android, la ruta y Ajustes de televisor.
/// Con el reparto en un solo lugar no puede quedar una entrada abriendo la
/// pantalla equivocada, que es exactamente lo que pasó hasta ahora: en
/// televisor se veía la de teléfono con dos retoques.
///
/// Y con un envoltorio sin estado, la que NO corresponde ni siquiera se
/// construye. Devolviendo desde el `build` de la de teléfono se armaría
/// igual su controlador, sus dos controladores de página y un
/// `ScrollController` por página, para tirarlos enseguida.
class ExtensionPage extends StatelessWidget {
  const ExtensionPage({super.key});

  @override
  Widget build(BuildContext context) => PlatformTv.esTelevisionSync
      ? const ExtensionPageTv()
      : const _ExtensionPageTactil();
}

/// La de teléfono, tablet y PC: lista paginada que se cambia deslizando,
/// chips de filtro arriba y las acciones masivas en un menú.
class _ExtensionPageTactil extends StatefulWidget {
  const _ExtensionPageTactil();

  @override
  State<_ExtensionPageTactil> createState() => _ExtensionPageState();
}

class _ExtensionPageState extends State<_ExtensionPageTactil>
    with AccionesMasivasDeExtensiones<_ExtensionPageTactil> {
  @override
  ...
  Widget _buildBarraDeFiltros() {
    ...
    final acciones = _BotonDeAccionesMasivas(
      enCurso: masivoEnCurso,
      onActivar: () => conElPasoCerrado(() => cambiarTodas(true)),
      onDesactivar: () => conElPasoCerrado(() => cambiarTodas(false)),
      onActualizar: () => conElPasoCerrado(actualizarTodas),
      onDesinstalar: () => conElPasoCerrado(desinstalarTodas),
    );
        ...
        return StatefulBuilder(
          builder: (hojaContext, setHoja) {
            Widget accion({
              ...
              return ListTile(
                enabled: !masivoEnCurso,
                leading: Icon(
                  masivoEnCurso ? Icons.hourglass_top_rounded : icono,
                ...
                onTap: () {
                  ...
                  unawaited(conElPasoCerrado(alTocar));
                },
              );
            }
