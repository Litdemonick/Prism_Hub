...
var extension = {
  latest: function(page) {
    ...
    var result = http.get('https://animeonlineninja.com/?page=' + page);
    var list = [];
    ...
    for (var item of items) {
      list.push({
        title: item[3],
        url: 'https://animeonlineninja.com' + item[1],
        image: item[2]
      });
    }
  ...
  search: function(kw, page, filter) {
    ...
    var result = http.get('https://animeonlineninja.com/search?q=' + encodeURIComponent(kw) + '&page=' + page);
    var list = [];
  ...
  detail: function(url) {
    ...
    var descMatch = result.match(/<p class="synopsis">([^<]+)<\/p>/);
    ...
    for (var ep of eps) {
      detail.episodes.push({
        title: ep[2],
        url: 'https://animeonlineninja.com' + ep[1]
      });
    }
